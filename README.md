...
An open store front for players to purchase ranks. These are configurable through the XenForo 2 admin control panel. Ranks will automatically sync to all Garry's Mod servers when the transaction is processed and the user's forum account has the secondary user group applied.

![Store Front](images/Store%20Main.png)

![Store Front Payment](images/Store%20Rank%20Selected.png)
...
To populate the forum with existing data, drop a `mysqldump` of your XF database at `mariadb/gmcore_forums.sql`. The initdb script [04-import-gmcore_forums.sh](mariadb/initdb/04-import-gmcore_forums.sh) imports it into `gmcore_forums` on the first MariaDB boot.
...
| --- | --- | --- |
| `gmcore_forums` (== `MARIADB_DATABASE`) | XenForo | XF install / upgrade |
