 ...
 *
 * Handles user linking via GModInterface add-on:
...
class Discord extends AbstractController
{
	 ...
	 *
	 * Uses the GModInterface add-on to look up active Discord↔Forum account links.
	 * Returns null if:
	 * - Discord ID is empty
	 * - GModInterface database is not configured
	...
	protected function tryGetLinkedForumUserId(string $discordUserId): ?int
	{
		...
		try {
			// GModInterface owns Discord↔Forum linking.
			$options = XF::options();
			...
			// If the core DB connection isn't configured, treat as unlinked.
			return null;
		}
	}
	 ...
	 *
	 * Prevents Discord users from abusing @everyone, @here, and other mention types
	 * in messages forwarded from Discord to the shoutbox.
	 *
	 * Patterns checked:
	 ...
	 * - Display problems in templates
	 * - Security issues from embedded nulls or control codes
	 *
	 ...
	 *
	 * Behavior:
	 * - Checks for duplicate discord_message_id (returns existing if found)
	 * - Validates message is not empty and doesn't contain Discord pings
	 * - Attempts to link Discord user to XenForo account via GModInterface
	 * - If linked: Posts as the XenForo user
	 * - If unlinked: Creates generic entry with Discord identity stored
	 * - Updates last_id cache and bumps generation for real-time sync
	 *
	 ...
	 *
	 * Behavior:
	 * - Finds message by discord_message_id
	 * - Deletes if found (no error if not found)
	 * - Recalculates last_id cache from MAX(id)
	...
	public function actionPostTimeout(ParameterBag $params): ApiResult|Error
	{
		...
		// Body fields supported (bot may send one or more):
		// - discord_user_id (preferred)
		// - moderator_discord_user_id (optional)
		...
		try {
			...
			if (!$ban) {
				...
				// Update banning user if we can resolve the moderator.
				if ($banUserId && (int)$ban->ban_user_id !== $banUserId) {
					$ban->ban_user_id = $banUserId;
					$changed = true;
				}

				// If already permanently shoutbox-banned, leave as-is.
				if ($ban->expires_on === null) {
					// Keep $changed as-is (ban_user_id might have been updated).
				} else {
	...
	public function actionPostUntimeout(ParameterBag $params): ApiResult|Error
	{
		...
		try {
			...
			// Deleting the ban will automatically archive into shoutbox_ban_history.
			$ban->delete();
