...
class Message extends Repository
{
	 ...
	 *
	 * Used for incremental polling - fetches only new messages since the last poll.
	 *
	 ...
	 *
	 * Used for long-polling optimization. If cache is empty (cold start),
	 * seeds the cache with the current MAX(id) from the database.
	 *
	 * @return int The highest message ID currently in the shoutbox
	 */
	public function getLastIdFromCache(): int
	{
		...
		// Cache cold-start: seed from DB so long-poll has the correct baseline.
	 ...
	 *
	 * Called after new messages are created to allow long-polling clients
	 * to quickly detect new messages without hitting the database.
	 *
	 * @param int $lastId The new highest message ID to cache
	 * @return void
	 */
	 ...
	 *
	 * @return int The new generation counter value
	 */
