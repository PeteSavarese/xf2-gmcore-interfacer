...
class Setup extends AbstractSetup
{
	...
	public function upgrade1010000Step1()
	{
		...
		if (!$sm->tableExists('shoutbox_discord_emoji')) {
			$sm->createTable('shoutbox_discord_emoji', function (Create $table) {
				$table->addColumn('name', 'varchar', 50);
				$table->addColumn('discord_id', 'varchar', 50);
				$table->addColumn('animated', 'tinyint')->setDefault(0);
				$table->addColumn('source', 'varchar', 16)->setDefault('ingest');
				$table->addColumn('last_seen', 'int')->setDefault(0);
				$table->addPrimaryKey('name');
				$table->addKey('discord_id');
			});
		}
	}
}
