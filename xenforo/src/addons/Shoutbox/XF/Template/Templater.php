...
class Templater extends XFCP_Templater
{
	public function addFunctions(array $functions)
	{
		...
		$functions['shoutbox_discord_emoji_to_img'] = 'fnShoutboxDiscordEmojiToImg';

		return parent::addFunctions($functions);
	}

	public function fnShoutboxDiscordEmojiToImg($templater, &$escape, $html)
	{
		$escape = false;
		if (!is_string($html) || $html === '') {
			return $html ?? '';
		}
		/** @var \Shoutbox\Service\DiscordEmoji\Transformer $transformer */
		$transformer = XF::service('Shoutbox:DiscordEmoji\\Transformer');
		return $transformer->toHtml($html);
	}
